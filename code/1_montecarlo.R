# ------------------------------------------------------------------------------
# Setup: Data Generating Process (DGP)
# ------------------------------------------------------------------------------

set.seed(43252)  # For reproducibility

# Sample size and parent income distribution
N <- 5000
x <- exp(runif(N, 0, 12))  # Parental income on level scale (exponentiated for right skew)

# Mobility threshold: child must earn 15% more/less than parent to be considered mobile
bw <- log(1.15)

# Group-specific parameters for log-linear model: log(y) = alpha + beta*log(x) + error
# Group 0 (higher persistence): steeper slope, lower intercept
alpha0 <- 1.5; beta0 <- 0.70; gamma0 <- 0.7; lambda0 <- -0.1

# Group 1 (lower persistence): flatter slope, higher intercept  
alpha1 <- 2.5; beta1 <- 0.35; gamma1 <- -0.5; lambda1 <- 0.1
...
mob_types <- c("up", "down", "imm")  # Upward, downward, immobile
...
# Heteroskedastic errors: variance depends on log(x) with some noise
sigma0_base <- gamma0 + lambda0 * log(x)
sigma1_base <- gamma1 + lambda1 * log(x)

sigma0 <- exp(sigma0_base + rnorm(N, 0, 0.2))
sigma1 <- exp(sigma1_base + rnorm(N, 0, 0.2))

# Generate child income for each group
y0 <- exp(rnorm(N, mean = alpha0 + beta0 * log(x), sd = sigma0))
df_sim0 <- tibble(group = 0, x = x, y = y0)

y1 <- exp(rnorm(N, mean = alpha1 + beta1 * log(x), sd = sigma1))
df_sim1 <- tibble(group = 1, x = x, y = y1)

# Combine groups and classify mobility type
df_sim <- bind_rows(df_sim0, df_sim1) %>%
  mutate(mobility_type = case_when(
    y / x > exp(bw) ~ "up",      # Child earns >15% more
    x / y > exp(bw) ~ "down",    # Child earns >15% less
    TRUE ~ "imm"                 # Child earns within 15% of parent
  ))
...
# Reference point for log-centering income (helps with numerical stability)
x_ref_log <- min(log(df_sim$x))

# Create evaluation grid for counterfactual predictions
# We'll predict mobility at 8 income levels spanning the data range
log_grid <- seq(
  quantile(log(df_sim$x) - x_ref_log, .01),
  quantile(log(df_sim$x) - x_ref_log, .99),
  length.out = 8
)

# Store bounds for later use
assign("floor_mc", x_ref_log) 
assign("lb_mc", min(log_grid)) 
assign("ub_mc", max(log_grid)) 

# Back-transform to level scale and repeat for both groups
x_new <- exp(log_grid + x_ref_log)
x_new <- rep(x_new, times = 2) %>% round(0)

group_new <- rep(0L:1L, each = length(log_grid))  # 0,0,0,...,1,1,1
...
model_stan <- paste0("glm_mobility_model.stan")
mod <- cmdstan_model(here("code", model_stan), force_recompile = TRUE)

# Run MCMC sampling (chain/iteration counts assumed defined in 0_masterfile)
fit <- mod$sample(
  data = stan_data,
  chains = n_chains,
...
# Extract posterior draws for all quantities of interest
...
posterior <- fit$draws(variables = vars, format = "draws_df")

# ------------------------------------------------------------------------------
# Process posterior mobility probabilities
# ------------------------------------------------------------------------------

# Initialize results table with prediction grid
result <- tibble(x = x_new, group = group_new)

# For each scenario and mobility type, extract posterior summaries
for (s in scenarios) {
  for (m in mob_types) {
    var <- paste0(m, "_", s)
...
# Convert to long format for stacked area plots
result_long_all <- result %>%
  ...
  mutate(
    x_log = round(log(x) - x_ref_log, 1),
    group_label = factor(group, levels = 0:1, labels = c("Group~A*' | '*theta[b]", "Group~B*' | '*theta[a]")),
...
# Marginal probabilities (averaged over income distribution)
...
# Small marginal plot showing overall distribution
...
p1 <- ggplot(result_long_all, aes(x = round(x_log, 1), y = mean, fill = type, group = type)) +
  ...
  facet_grid(group_label ~ scenario, labeller = labeller(group_label = label_parsed)) +
  labs(
...
ggsave(here("figures", paste0("mobility_area_plot.png")),
...
diff_long <- diff_result %>%
  ...
  mutate(
    group = factor(group, levels = 0:1, labels = c("Group~A*' | '*theta[b]", "Group~B*' | '*theta[a]")),
    type = factor(type, levels = c("up", "imm", "down"), labels = c("Upward", "Immobile", "Downward")),
...
ggsave(here("figures", paste0("mobility_diff_plot.png")),
...
# Extract log-likelihood for LOO-CV and WAIC
...
assign("parameters_mc", param_summary_kbl)  
...
writeLines(latex_table_params,
           here("tables", paste0("posterior_params_summary.tex")))

# ==============================================================================
# Aggregation and Composition Decomposition
# ==============================================================================
# This section decomposes mobility gaps into:
# 1. Aggregation effect: difference due to parameters (holding x-distribution fixed)
# 2. Composition effect: difference due to x-distribution (holding parameters fixed)
# ==============================================================================

# Setup prediction grid for decomposition
grid_df <- tibble(
  idx = seq_along(x_new),
  group = group_new,
  x = x_new
...
# ------------------------------------------------------------------------------
# We need p_g(x) = the empirical density of income in each group
# These weights let us compute expected mobility over the income distribution
...
group_densities <- grid_df %>%
  ...
  group_modify(~ {
    ...
    if (length(x_emp) < 2L) {
      ...
      dens_vals <- approx(dens$x, dens$y, xout = .x$x_eval, rule = 2)$y
    }
...
p_weights <- grid_df %>%
  mutate(x_eval = x) %>%
  left_join(group_densities, by = c("group", "x_eval")) %>%
...
# ------------------------------------------------------------------------------
# For each group, compute weighted average of (CF - Obs) using own x-distribution
# This tells us: "If we swapped the parameters but kept the income distribution 
# the same, how would mobility change?"
...
for (g in 0:1) {
  ...
  for (sc in scenario_short) {
    for (tp in mobility_levels) {
      ...
      # Extract posterior draws for this scenario
      ...
      agg_rows[[length(agg_rows) + 1]] <- tibble(
        ...
        Group = ifelse(g == 0, "Group A", "Group B"),
        Scenario = case_when(
...
agg_table_df_sim <- bind_rows(agg_rows)

# ------------------------------------------------------------------------------
# 2. COMPOSITION EFFECT
# ------------------------------------------------------------------------------
# For each group, compute weighted difference using OTHER group's x-distribution
# This tells us: "If we changed the income distribution to match the other group
# but kept our parameters, how would mobility change?"
...
for (g in 0:1) {
  ...
  # Get own group's grid and weights
  idx_g <- grid_df %>% filter(group == g) %>% arrange(idx) %>% pull(idx)
  w_g <- p_weights %>% filter(group == g) %>% arrange(idx) %>% pull(p)
  ...
  # Normalize and compute difference in weights
...
comp_table_df_sim <- bind_rows(comp_rows) %>% arrange(Group, Mobility)
...
combined_agg_comp_table <- bind_rows(
  agg_table_df_sim %>% mutate(Analysis = "Aggregation"),
  comp_table_df_sim %>% mutate(Analysis = "Composition")
) %>%
...
plot_data <- combined_agg_comp_table %>%
  filter(Analysis %in% c("Aggregation", "Composition")) %>%
  mutate(
    ...
    # Ensure error bars display correctly when mean crosses zero
    ymin_err = ifelse(Mean < 0, `Lower 95%`, Mean),
    ymax_err = ifelse(Mean > 0, `Upper 95%`, Mean),
    ymin_err = ifelse(Mean >= 0 & `Lower 95%` < 0, `Lower 95%`, ymin_err),
    ymax_err = ifelse(Mean <= 0 & `Upper 95%` > 0, `Upper 95%`, ymax_err)
  ) %>%
  arrange(Analysis, Group, x_label)
...
ggsave(here::here("figures", paste0("combined_agg_comp_effects.png")),
