// ==============================================================================
// GLM Mobility Model with Measurement Error Adjustment
// ==============================================================================
// Estimates group-specific log-linear income models with heteroskedasticity
// and measurement error correction, then computes counterfactual mobility
// probabilities under parameter swaps.
//
// Model: log(y) = alpha[g] + beta[g] * log(x) + error
//        error ~ N(0, sigma_total[g](x))
//        sigma_total accounts for both structural and measurement error
//
// Measurement error: Adjusts for within-individual income volatility based on
//                    number of observations per person (T_obs) and empirical SD
//
...
data {
  ...
  vector<lower=0>[N] logy_sd;                  // Within-individual income volatility (log scale)
  vector<lower=1>[N] T_obs;                    // Number of income observations per person
...
transformed parameters {
  vector<lower=0>[N] sigma_total;  // Total variance (structural + measurement)
  ...
  // Expected log(y) and structural standard deviation
  vector[N] mu;
  ...
  // Combine structural and measurement error variance
  // sigma_total^2 = sigma^2 + (measurement error variance)
  // Measurement error variance = (logy_sd / sqrt(T_obs))^2
  for (i in 1:N) {
    sigma_total[i] = sqrt(square(sigma[i]) + square(logy_sd[i] / sqrt(T_obs[i])));
  }
}
...
model {
  ...
  // Likelihood (uses total variance including measurement error)
  for (i in 1:N) {
    y[i] ~ lognormal(mu[i], sigma_total[i]);
  }
}
...
generated quantities {
  ...
  // Note: Counterfactuals use structural sigma only (no measurement error)
  ...
  // Generate predictions for observed data (includes measurement error)
  for (i in 1:N) {
    y_pred[i] = lognormal_rng(mu[i], sigma_total[i]);
    residual[i] = log(y[i]) - mu[i];
    log_lik[i] = lognormal_lpdf(y[i] | mu[i], sigma_total[i]);
  }

  // Compute mobility probabilities for counterfactual scenarios
  // Uses structural sigma only (measurement error not part of counterfactuals)
  for (j in 1:N_new) {
    ...
    // Observed parameters (own group, structural variance only)
